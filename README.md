...
`doctor --json` is the setup diagnostic payload: writable storage, osascript availability, active-app probe status/error, idle probe status/error, storage verification, paths, and permission hints. Probe failures are reported as fields instead of making the whole command unusable.
