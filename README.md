...
`repair-context` repairs high-confidence browser title/URL mismatches and missing browser context from immediate neighbor evidence or a unique clean exact-URL title observation, converts unrecoverable all-missing browser context rows and short unrecoverable mixed-context rows to untracked time, then recomputes category.
