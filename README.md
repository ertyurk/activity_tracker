...
`audit --json` reports log quality for a day: gaps above a configurable threshold, overlaps, invalid rows, missing titles, browser sessions missing URLs, untracked/idle counts, uncategorized counts, by-app/by-title quality breakdowns, and current open-session state.
